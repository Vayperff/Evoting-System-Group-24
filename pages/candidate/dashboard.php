...
?>
  ...
  <!-- Dashboard Header -->
  <div class="ms-3">
    <h3 class="mb-0 h4 font-weight-bolder">Elections you are paticipating in</h3>
  ...
  <div class="col-lg-6 col-7">
    ...
    <p class="text-sm mb-0">
      ...
      <span class="font-weight-bold ms-1"><?= number_format($rescandidates->num_rows); ?> elections </span> currently active
    </p>
  </div>
  ...
  <thead>
    <tr>
        ...
        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Election status</th>
 ...
 <tbody>
<?php while($row1 = $rescandidates->fetch_object()) { ?>
<?php 
$sqlElectionId = $row1->election_id;

$sql = $mysqli->prepare("SELECT * FROM elections WHERE deleted != 'DELETED' AND status != 'COMPLETED' AND id = ?  ORDER BY date_created DESC");
$sql->bind_param("i",$sqlElectionId);
$sql->execute();
$elections = $sql->get_result();
$row = $elections->fetch_object();

 $sqlElectionId = filter_var($row->id, FILTER_SANITIZE_NUMBER_INT);
 $sqlCandidates = $mysqli->prepare("SELECT * FROM candidates WHERE election_id = ? AND deleted != 'DELETED' AND election_status = 0 AND email = ?");
 $sqlCandidates->bind_param("is", $sqlElectionId,$email1);
 $sqlCandidates->execute();
 $resCandidates = $sqlCandidates->get_result();
        
 $sqlvotes = $mysqli->prepare("SELECT * FROM votes WHERE voting_status = '0' AND election_id = ? AND email  = ?");
...
?>
 ...
 <?php
        $sqlElectionId = filter_var($row->id, FILTER_SANITIZE_NUMBER_INT);
        $sqlCandidates = $mysqli->prepare("SELECT * FROM candidates WHERE election_id = ? AND deleted != 'DELETED' AND email = ?");
        $sqlCandidates->bind_param("is", $sqlElectionId,$email1);
        $sqlCandidates->execute();
        $resCandidates = $sqlCandidates->get_result();
            ...
            <td>
                <div class="d-flex px-2 py-1">
                    <div class="d-flex flex-column justify-content-center">
                        ...
                        <!--
                            (<?= htmlspecialchars(number_format($resCandidates->num_rows)); ?>)
                            -->
                        ...
                        <?php  if($currentDate === $endDateForElections): ?>
                            <button class="text-xs text-success  mb-0" 
                                ...
                                ?>)   Final vote count for this election
                                -->
                                View votes u have for this election
                            </button>
                            <?php else: ?>
                            <button class="text-xs text-muted mb-0" 
                                ...
                                You have  (<?= number_format($resvotes->num_rows); ?>)  votes for this election
                            </button>
                            <?php endif; ?>
                        </p>
                    </div>
...
<div class="modal fade" id="imageModal_<?= $row->id; ?>" tabindex="-1" aria-labelledby="imageModalLabel_<?= $row->id; ?>" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body text-center">
                <img src="../images/candidate_images/<?= htmlspecialchars($row->election_banner); ?>" 
    ...
    ?>
        ...
        <?php else: ?>
            ...
            <p class="text-xs font-weight-bold text-success">Election is being reviewed</p>
            <!--
    ...
    <div class="col-md-4 mb-3">
        <div class="card shadow-sm border rounded p-3 d-flex flex-row align-items-center">
            <div class="text-left me-3">
                ...
                <p class="text-xs text-muted">You have <strong> (<?= number_format($resCountVotes->num_rows); ?>) votes </strong> for this election</p>
            </div>
