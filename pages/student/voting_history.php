...
?>
  ...
  <!-- Dashboard Header -->
  <div class="ms-3">
    <h3 class="mb-0 h4 font-weight-bolder">Elections you voted in</h3>
...
*/ ?>
  ...
  <div class="col-lg-6 col-7">
    <h6>Elections you voted in</h6>
    <p class="text-sm mb-0">
      <i class="fa fa-check text-info" aria-hidden="true"></i>
      <span class="font-weight-bold ms-1"><?= number_format($electionsCompletedVoteHistory->num_rows); ?> elections </span> completed
    </p>
  </div>
 ...
 <tbody>
    <?php while($row1 = $electionsCompletedVoteHistory->fetch_object()) { ?>
        <?php
        
        $electID = $row1->election_id;
        
        $sqlCompletedVH = $mysqli->prepare("SELECT * FROM elections WHERE id = ? AND  deleted != 'DELETED'  AND status = 'COMPLETED' ORDER BY date_created DESC");
        $sqlCompletedVH->bind_param("i", $electID);
        
        $sqlCompletedVH->execute();
        $electionsCompletedVH = $sqlCompletedVH->get_result();
        $row = $electionsCompletedVH->fetch_object();
        ...
        $sqlvotes = $mysqli->prepare("SELECT * FROM votes WHERE voting_status = '1' AND election_id = ?");
        ...
        <div class="card shadow-sm border rounded p-3 d-flex flex-row align-items-center">
            ...
            <div class="d-flex flex-column align-items-center">
               ...
               <span class="text-success">
                   Winner for this election <input type="checkbox" checked> <br>
