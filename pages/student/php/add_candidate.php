...
if (isset($_POST['add_candidate1'])) {
    ...
    $sqlEmail = $mysqli->prepare("SELECT * FROM candidates WHERE email = ? AND deleted != 'DELETED' ");
 ...
 $deleted = "active";
    ...
    $sql = $mysqli->prepare("INSERT INTO candidates (candidate_name, candidate_role, candidate_bio, email, candidate_image, date_format, election_id,deleted) VALUES (?, ?, ?, ?, ?, ?, ?,?)");
    ...
    // Bind parameters
    $sql->bind_param("ssssssss", $candidate_name, $candidate_role, $candidate_bio, $email, $image, $date, $election_id,$deleted);
    ...
    if (!$sql->execute()) {
        ...
        try {
           // Server settings
            ...
            // Send email
            if ($mail->send()) {
                echo "<script>alert('Candidate details updated successfully, Notification sent'); document.location.href = 'manage_candidates.php';</script>";
            } else {
                echo 'Message could not be sent. Mailer Error: ' . $mail->ErrorInfo;
            }
        } catch (Exception $e) {
