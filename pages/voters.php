...
$sqlusers = $mysqli->prepare("SELECT * FROM student_sign_up WHERE deleted = 0 ORDER BY date_created DESC");
$sqlusers->execute();
$resuser = $sqlusers->get_result();

if(isset($_POST['delete_user'])) {
    $id = $mysqli->real_escape_string(filter_var($_POST['id'], FILTER_SANITIZE_NUMBER_INT));
    $names1 = $mysqli->real_escape_string(htmlspecialchars($_POST['names1'], ENT_QUOTES, 'UTF-8'));
    $email = $mysqli->real_escape_string(htmlspecialchars($_POST['email'], ENT_QUOTES, 'UTF-8'));

    $status = 1;
    $sql = $mysqli->prepare("UPDATE student_sign_up SET deleted = ? WHERE id = ?");
    $sql->bind_param("ii", $status, $id);

    if ($sql->execute()) {  // **Email should be sent on success**
        $date = date("d, M Y"); // Fix undefined $date
        ...
        try {
            ...
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Try changing this if ENCRYPTION_SMTPS fails
            $mail->Port = 587; // Change to 465 if using SMTPS

            $mail->setFrom('info@skyratesipifalls.com', 'Evoting System Group 24');
            $mail->addAddress($email);

            $mail->isHTML(true);
            $mail->Subject = 'Account Removal';
            $mail->Body = "<h4>Hello $names1, your account has been removed from the system.</h4><br>Date: $date<br>";

            if ($mail->send()) {
                echo "<script>
                        alert('Account successfully deleted and email sent.');
...
<?php include_once("navbar.php"); ?>
  ...
  <!-- Dashboard Header -->
  <div class="ms-3">
    <h3 class="mb-0 h4 font-weight-bolder">Voters  <span class="text-info"> 
...
*/ ?>
  ...
  <thead>
    <tr>
      ...
      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Remove user</th>
      
    </tr>
  </thead>
  <tbody>
    <?php if($resuser->num_rows > 0): ?>
      <?php while($row =  $resuser->fetch_object()) { ?>
        ...
        <tr>
          ...
          <td>
              ...
              <form method="post">
                  ...
                  <input type="hidden" name="names1" value="<?= $row->username; ?>">
