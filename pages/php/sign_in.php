...
if($_SERVER["REQUEST_METHOD"] === "POST") {
	if(isset($_POST['login'])) {
		...
		$sql = $mysqli->prepare("SELECT * FROM admin_sign_up WHERE email = ? OR reg_name = ? AND status != 'DELETED'");
		...
		if($fetch) {
			if(password_verify($password,$fetch->password)) {
				...
				$_SESSION['admin_id'] = filter_var($fetch->id,FILTER_SANITIZE_NUMBER_INT);
