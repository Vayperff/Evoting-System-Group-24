...
*/ ?>
  ...
  <div class="col-lg-6 col-7">
    <h6>Elections completed</h6>
    <p class="text-sm mb-0">
      <i class="fa fa-check text-info" aria-hidden="true"></i>
      <span class="font-weight-bold ms-1"><?= number_format($rescandidates2->num_rows); ?> elections </span> completed
    </p>
  </div>
 ...
 <tbody>
    <?php while($row1 = $rescandidates2->fetch_object()) { ?>
        <?php
        $sqlElectionId = filter_var($row1->election_id, FILTER_SANITIZE_NUMBER_INT);
        
        
        
        
        $sqlCompleted = $mysqli->prepare("SELECT * FROM elections WHERE id = ? AND  deleted != 'DELETED'  AND status = 'COMPLETED' ORDER BY date_created DESC");
        $sqlCompleted->bind_param("i",$sqlElectionId);
        $sqlCompleted->execute();
        $electionsCompleted = $sqlCompleted->get_result();
        $row = $electionsCompleted->fetch_object();
        ...
        $sqlvotes = $mysqli->prepare("SELECT * FROM votes WHERE voting_status = '1' AND election_id = ? AND email = ?");
        $sqlvotes->bind_param("is",$sqlElectionId,$email1);
        $sqlvotes->execute();
        ...
        $endDateForElections = $row->end_date;
         ...
         if($currentDate !== $endDateForElections) {
             ...
             $sqlUpdate->bind_param("si",$status,$sqlElectionId);
             if(!$sqlUpdate->execute()) {
                 die("Error");
             }
             
             $sqlupdatevotes = $mysqli->prepare("UPDATE votes SET voting_status = ? WHERE election_id = ?");
             $sqlupdatevotes->bind_param("ii",$voting_status,$sqlElectionId);
             
             if(!$sqlupdatevotes->execute()) {
                 die("Error");
             }
                    ...
                    <div class="d-flex flex-column justify-content-center">
                        ...
                        <p class="text-xs text-muted mb-0 text-center"> 
                            <button class="text-xs text-muted mb-0" 
                                ...
                                View (<?= number_format($resvotes->num_rows); ?>)
                                votes for this candidates
                            </button>
                        </p>
                    </div>
                </div>
...
$sqlMaxVotes = $mysqli->prepare("
    ...
    FROM votes 
    WHERE election_id = ? AND voting_status = 1 
...
if ($maxVote) {
    ...
    if ($winnerCount == 1) {
        ...
        if ($candidate) {
           echo '<p class="text-xs font-weight-bold text-success">
        The candidate with <br> who won election <br> for the role of <br> 
      ...
      <button class="text-xs text-success mb-0" 
        ...
        // Step 3B: If there's a tie, list all tied candidates
        echo '<p class="text-xs font-weight-bold text-danger">There is a tie between the following candidates:</p><ul class="text-danger">';
        ...
        echo '</ul><p class="text-xs font-weight-bold text-danger">
        Election resolved manually</p>';
    }
}
                ...
                <form method="post">
                    ...
                    <button disabled class="btn btn-sm btn-success" type="submit" name="complete_elections">
                        Elections completed
                    </button>
                </form>
                <?php endif; ?>
    ...
    // Get top voted candidate ID for this election
$winnerStmt = $mysqli->prepare("
    ...
    LIMIT 1
");
$winnerStmt->bind_param("i", $electionID);
$winnerStmt->execute();
$winnerResult = $winnerStmt->get_result();
$winnerData = $winnerResult->fetch_object();

$topCandidateID = isset($winnerData->candidate_id) ? $winnerData->candidate_id : null;





    $sqlCountVotes = $mysqli->prepare("SELECT * FROM votes WHERE candidate_id = ? AND election_id = ? AND voting_status = 1");
    ...
    $sqlCheckVote = $mysqli->prepare("SELECT * FROM votes WHERE election_id = ? AND user_id = ? AND status = 'VOTED' AND voting_status = 1");
    ...
    $sqlCheckVote1 = $mysqli->prepare("SELECT * FROM votes WHERE election_id = ? AND user_id = ? AND status = 'VOTED' AND candidate_id = ? AND voting_status = 1");
    ...
    <form method="get" action="view_voters1.php">
        ...
        <br>
        <?php if($candidateID == $topCandidateID): ?>
            <span class="text-success">
                Winner for this election <input type="checkbox" checked>
            </span>
        <?php endif; ?>
    </form>
<?php else: ?>
...
<div id="popupContainer" class="popup">
    ...
    function updateCountdown(id, endDate) {
        function calculateTimeLeft() {
            ...
            if (timeLeft <= 0) {
                document.getElementById(id).innerHTML = "Election period done";
